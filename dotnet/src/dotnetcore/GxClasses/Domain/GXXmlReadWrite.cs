	...
	public class GXXMLReader : IDisposable
	{
		...
		private XmlReader Reader
		{
			get
			{
				if (treader == null)
				{
					treader = OpenXMLTextReader();
				}
				return treader;
			}
		}
		private Object XMLInput;
		private XmlReaderSettings treaderSettings = new XmlReaderSettings();
		...
		public void ValidationCallBack( object sender, ValidationEventArgs args )
		{
			...
			if (treaderSettings != null)
			{
				IXmlLineInfo lineInfo = treader as IXmlLineInfo;
				if (lineInfo != null)
				{
					errorLineNumber = lineInfo.LineNumber;
					errorLinePos = lineInfo.LinePosition;
				}
			}
			else
			{
		...
		private void CreateXmlTextReader (string URL)
		{		
			try
			{
				if (documentEncoding == null) 
				{
					using (XmlTextReader auxReader = new XmlTextReader(URL))
					{
...
#pragma warning disable SYSLIB0014 // WebRequest
		...
		{
			treaderSettings = new XmlReaderSettings() { XmlResolver = Resolver };
			treaderSettings.ValidationEventHandler += new ValidationEventHandler(ValidationCallBack);
			SetDtdProcessing(treaderSettings, Resolver, validationType);
		...
		public short Open(string URL1)
		{
			...
			CreateXMLSettings();
			CreateXmlTextReader(URL);
			return 0;
		}

		private void SetDtdProcessing(XmlReaderSettings treaderSettings, GXResolver resolver, int validationType)
		{
			...
			for (i = 0; i < schemas.Count; i++)
			{
				try
				{
					...
					if (!String.IsNullOrEmpty(uri))
						treaderSettings.Schemas.Add(nameSpace, uri);
				}
				catch (Exception e)
				{
					errorCode = 1;
		...
		public void AddSchema(string uri, string nameSpace)
		{
			...
			if (treaderSettings != null && !String.IsNullOrEmpty(uri))
			{
				try
				{
                    if (String.IsNullOrEmpty(nameSpace))
						treaderSettings.Schemas.Add(null, uri);
					else
						treaderSettings.Schemas.Add(nameSpace, uri);
				}
				catch (Exception e)
		...
		public int RemoveWhiteNodes
		{
			...
			set
			{
				...
				if (treaderSettings != null)
					treaderSettings.IgnoreWhitespace = (value > 0);
			}
		}
			...
			set
			{
				if (value >= ValidationNone && value <= ValidationXDR)
				{
					...
					if (treaderSettings != null)
						...
						{
							case ValidationNone:
								treaderSettings.ValidationType = System.Xml.ValidationType.None;
								break;
							case ValidationAuto:
								treaderSettings.ValidationType = System.Xml.ValidationType.Auto;
								break;
							case ValidationDTD:
								treaderSettings.ValidationType = System.Xml.ValidationType.DTD;
								break;
							case ValidationSchema:
								treaderSettings.ValidationType = System.Xml.ValidationType.Schema;
								break;
							case ValidationXDR:
								treaderSettings.ValidationType = System.Xml.ValidationType.XDR;
								break;
						}
				}
			}
		...
		public int LinesNormalization
		{
			...
			set
			{
				XmlTextReader xmlTextReader = treader as XmlTextReader;
		...
		private short fillQueue()
		{
			...
			while (nRet && !StoppingError && getNodeExtent() == 0)
			{
				...
				if (nRet && !StoppingError)
				{
					...
					{	
						inContent = true;
						elementNode[nextElementNode].Name = Reader.Name;
						elementNode[nextElementNode].LocalName = Reader.LocalName;
						elementNode[nextElementNode].Prefix = Reader.Prefix;
						elementNode[nextElementNode].NameSpaceURI = Reader.NamespaceURI;
						elementNode[nextElementNode].Value = "";
						elementNode[nextElementNode].ClearAttributes();

						for (bool exists = Reader.MoveToFirstAttribute(); exists; exists = Reader.MoveToNextAttribute())
							elementNode[nextElementNode].AddAttribute(Reader.Name, Reader.Prefix, Reader.LocalName, Reader.NamespaceURI, Reader.Value);
						...
						{
							endTagNode.Name = Reader.Name;
							endTagNode.LocalName = Reader.LocalName;
							endTagNode.Prefix = Reader.Prefix;
							endTagNode.NameSpaceURI = Reader.NamespaceURI;
					...
					{
						piNode.Name = Reader.Name;
						piNode.Value = Reader.Value;
					...
					else if (Reader.NodeType == XmlNodeType.CDATA) 
					{
						...
						valuedNode[nextValuedNode].Value = Reader.Value;
					...
					else if (Reader.NodeType == XmlNodeType.DocumentType) 
					{
						...
						valuedNode[nextValuedNode].Value = Reader.Name;
						NodesQueue.Add(valuedNode[nextValuedNode]);
						nextValuedNode = (nextValuedNode + 1) % QUEUE_SIZE;
						if (!String.IsNullOrEmpty(Reader.Value))
							EntitiesContainer.addString(Reader.Value);
					}
					else if (Reader.NodeType == XmlNodeType.Comment && !ignoreComments) 
					{
					...
					{
						bool Empty = String.IsNullOrEmpty(Reader.Value.Trim());
					...
					else if (Reader.NodeType == XmlNodeType.SignificantWhitespace || Reader.NodeType == XmlNodeType.Whitespace)
					{
		...
		private XmlReader OpenXMLTextReader()
		{
			Stream stream = XMLInput as Stream;
			string uri = XMLInput as string;
			StringReader strReader = XMLInput as StringReader;
			try
			{
				if (stream != null)
					return XmlReader.Create(stream, treaderSettings);
				else if (strReader != null)
					return XmlReader.Create(strReader, treaderSettings);
				else
					return XmlReader.Create(uri, treaderSettings);
			}
			catch (ArgumentNullException)
			{
				errorCode = 1;
				errorDescription = "Bad URL" + (!(string.IsNullOrEmpty(uri))? ": " + uri : string.Empty);
				errorLineNumber = 0;
				errorLinePos = 0;
				return null;
			}
...
#region IDisposable Members
        ...
        public void Dispose()
        {
			if ( Reader != null || treaderSettings != null )
                this.Close();
        }
