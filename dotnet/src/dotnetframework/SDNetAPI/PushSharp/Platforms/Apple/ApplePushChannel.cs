...
namespace PushSharp.Apple
{
	public class ApplePushChannel : PushChannelBase
	{
		...
		public ApplePushChannel(ApplePushChannelSettings channelSettings, PushServiceSettings serviceSettings ) : base(channelSettings, serviceSettings)
		{           
		...
		/*	taskCleanup = new Task(() => Cleanup(), TaskCreationOptions.LongRunning);
			...
			taskCleanup.Start();
         * */
            taskCleanup = new Thread(new ThreadStart(Cleanup));
            taskCleanup.Start();
		}
		...
		{            
			...
			byte[] notificationData = Array.Empty<byte>();
		...
		public override void Stop(bool waitForQueueToDrain)
		{
			...
			{
				while (QueuedNotificationCount > 0 || sentNotifications.Count > 0 || PendingNotificationsResult > 0 )
					Thread.Sleep(500);
			}            
			...
			while (!this.CancellationRequested)
			{
				...
				lock (sentLock)
				{
					...
					if (sentNotifications.Count > 0)
					{
						...
						if (connected || this.CancellationRequested/*|| CancelToken.IsCancellationRequested*/)
						{
