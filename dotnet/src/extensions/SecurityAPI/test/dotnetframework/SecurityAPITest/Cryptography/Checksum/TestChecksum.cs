...
namespace SecurityAPITest.Cryptography.Checksum
{
	[TestFixture]
	public class TestChecksum : SecurityAPITestObject
	{
		...
		protected virtual void SetUp()
		{
			...
			localFileInput = Path.Combine(BASE_PATH, "Temp", "bookSample.xml");
			...
			results0x = new string[] { "0xF4", "0xDA", "0x15", "0xBC", "0x97", "0x7E", "0xA1", "0xA1", "0xD0", "0x25", "0xE5CC", "0x29B1", "0xBB3D",
				"0xFEE8", "0x4C06", "0x9ECF", "0x007E", "0x007F", "0xEA82", "0xC2B7", "0xD64E", "0x44C2", "0x6F91", "0x63D0", "0xD0DB", "0x0FB3",
				"0x26B1", "0xB4C8", "0xBF05", "0x2189", "0x4B37", "0x906E", "0x31C3", "0xCBF43926", "0xFC891918", "0xE3069283", "0x87315576",
				"0x0376E6E7", "0x765E7680", "0x3010BF7F", "0x340BC6D9", "0xBD0BE338", "0x25F9E794323B453885F5181F1B624D0B",
		...
		public void TestBulkChecksum()
		{
			...
			for (int j = 0; j < inputs.Length; j++)
			{
				for (int i = 0; i < algorithms.Length; i++)
				{
					...
					bool verify = check.VerifyChecksum(inputs[j], inputTypes[j], algorithms[i], checksum);
					True(verify, check);
				}
			}
		...
		public void TestBulkChecksum0x_LocalFile()
		{
			...
			for (int k = 0; k < algorithms.Length; k++)
			{
				string checksum = check.GenerateChecksum(localFileInput, "LOCAL_FILE", algorithms[k]);
				Assert.IsTrue(SecurityUtils.compareStrings(checksum, resultsLocalFile[k]));
				bool verify = check.VerifyChecksum(localFileInput, "LOCAL_FILE", algorithms[k],
						resultsLocalFile0x[k].ToLower());
				True(verify, check);
			}
		}
	}
