...
{
    [TestFixture]
    public class TestHmac: SecurityAPITestObject
    {
		...
		public virtual void SetUp()
		{
			...
			arrayHashes = new string[] { "MD5", "SHA1", "SHA224", "SHA256", "SHA384", "SHA512", "BLAKE2B_224",
				...
				"SHA3-224", "SHA3-256", "SHA3-384", "SHA3-512", "SM3", "TIGER", "WHIRLPOOL" };
			...
			arrayResHashes = new string[] { MD5, SHA1, SHA224, SHA256, SHA384, SHA512, BLAKE2B_224, BLAKE2B_256,
				...
				RIPEMD128, RIPEMD160, RIPEMD256, RIPEMD320, SHA3_224, SHA3_256, SHA3_384, SHA3_512, SM3, TIGER, WHIRLPOOL };
			...
			for (int i = 0; i < encodings.Length; i++)
			{
				...
				for (int a = 0; a < arrayHashes.Length; a++)
				{
					if (SecurityUtils.compareStrings(encodings[i], "UTF_8"))
					{
						...
						Assert.IsTrue(SecurityUtils.compareStrings(res, arrayResHashes[a]));
						True(true, mac);
					}
					string result = mac.calculate(plainText, password, arrayHashes[a]);
					Assert.IsTrue(mac.verify(plainText, password, result, arrayHashes[a]));
					True(true, mac);
				}
			}
		}
