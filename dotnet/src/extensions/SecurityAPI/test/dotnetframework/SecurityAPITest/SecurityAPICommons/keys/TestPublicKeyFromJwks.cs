...
{
	[TestFixture]
	public class TestPublicKeyFromJwks : SecurityAPITestObject
	{
		...
		public void testAzure()
		{
			...
			bool loaded = key.FromJwks(azure, akid);
		...
		{
			PublicKey key = new PublicKey();
			bool loaded = key.FromJwks(googlev2, gv2kid);
			Assert.IsTrue(loaded);
			Assert.IsFalse(key.HasError());
		}
		...
		{
			PublicKey key = new PublicKey();
			bool loaded = key.FromJwks(googlev3, gv3kid);
			Assert.IsTrue(loaded);
			Assert.IsFalse(key.HasError());
		}
		...
		{
			PublicKey key = new PublicKey();
			bool loaded = key.FromJwks(facebook, fkid);
			Assert.IsTrue(loaded);
			Assert.IsFalse(key.HasError());
		}
	}
