...
namespace GamSaml20Net
{
	public class RedirectBinding : IBinding
	{
		...
		public void Init(string queryString)
		{
			...
			string xml = System.Text.Encoding.UTF8.GetString(GamSaml20Net.Utils.Encoding.DecodeAndInflateXmlParameter(this.redirectMessage["SAMLResponse"]));
		...
		public static string Login(SamlParms parms, string relayState)
		{
			...
			XElement request = GamSaml20Net.Utils.SamlAssertionUtils.CreateLoginRequest(parms.Id, parms.Destination, parms.Acs, parms.Issuer, parms.PolicyFormat, parms.AuthContext, parms.SPname, parms.ForceAuthn);
		...
		public static string Logout(SamlParms parms, string relayState)
		{
			...
			XElement request = GamSaml20Net.Utils.SamlAssertionUtils.CreateLogoutRequest(parms.Id, parms.Issuer, parms.NameID, parms.SessionIndex, parms.Destination);
			return GenerateQuery(request, parms.Destination, parms.CertPath, parms.CertPass, relayState);
		}
		...
		private bool VerifySignature_internal(string certPath)
		{
			...
			byte[] signature = GamSaml20Net.Utils.Encoding.DecodeParameter(this.redirectMessage["Signature"]);
			string value;
		...
		private static string GenerateQuery(XElement request, string destination, string certPath, string certPass, string relayState)
		{
			...
			try
			{
				string samlRequestParameter = GamSaml20Net.Utils.Encoding.DelfateAndEncodeXmlParameter(request.ToString());
